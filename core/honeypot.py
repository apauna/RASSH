...
class HoneyPotCommand(object):
	...
	rassh.core.constants.current_command = self.honeypot.cmd

	print "start to get action .."
	action = self.getAction()
	if "initial_cmd" in CURRENT_CASE.keys():
	    case = CURRENT_CASE
	    case["next_cmd"] = self.honeypot.cmd
	    #self.write("save case ....\n")
	    print "saving the case ..."
	    getDB().saveCase(case)
	...
	if action == 0:
    	...
    	# Insult
    	    ...
    	    ret = getDB().getInsultMsg(location)
    	    self.write(ret)
    	elif action == 4:
    ...
    def getAction(self):
	print "getAction start", rassh.core.constants.current_command
	global RL
	RL.go()
	#return random.randint(0, 4)
	while True:
	    if rassh.core.constants.rl_action is not None:
	        print "********************", rassh.core.constants.rl_params
		return rassh.core.constants.rl_action[0]
	    else:
		print "action not foud. going to wait ....", rassh.core.constants.current_command
		time.sleep(1)
...
class HoneyPotShell(object):
    ...
    def runCommand(self):
        ...
        #proxy_mode.run_command(line, self.honeypot)
        #runOrPrompt()
        #return
        
        #self.honeypot.terminal.write("command cases handler\n")
