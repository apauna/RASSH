import rassh
from rassh.core import dblog
from rassh.core.config import *
...
class DB:
    ...
    def saveCase(self, case):
        print "case is saving", rassh.core.constants.rl_params
	cur=self.con.cursor()
	cmd = case["initial_cmd"]
	cur.execute("insert into cases(initial_cmd, action, next_cmd, cmd_profile, rl_params) values(%s, %s, %s, %s, %s)", (case["initial_cmd"], case["action"], case["next_cmd"], self.getProfile(cmd), str(rassh.core.constants.rl_params) ))
	self.con.commit();
